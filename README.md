...
| Package | For | Notes |
| --- | --- | --- |
| `POM2-v0.9.3-x86_64.AppImage` | Linux (Intel/AMD) | Runs on Mint 19+, Debian 12, Ubuntu 20.04+ (glibc **2.27** floor) |
| `POM2-v0.9.3-aarch64.AppImage` | Linux on ARM64 (desktop/server) | Desktop OpenGL, glibc **2.39** — Ubuntu 24.04+, Fedora 40+, Debian trixie |
| `POM2-v0.9.3-raspberry-aarch64.AppImage` | Raspberry Pi 3 → 5 | OpenGL **ES 3.0**, Raspberry Pi OS bookworm (glibc **2.36**). **Take this one on a Pi** |
| `POM2-v0.9.3-pi400-aarch64.AppImage` | Raspberry Pi 4 / Pi 400 only | Same, plus `-mcpu=cortex-a72` + PGO/LTO (**≈ −39 %** CPU). Will **not** start on an older core |
| `POM2-macOS-v0.9.3.dmg` | macOS 10.15+ | **Universal 2** — Apple Silicon *and* Intel in one file |
| `POM2-Windows-v0.9.3.zip` | Windows 10/11 x64 | One `POM2.exe`, no DLL beside it |
| `POM2-v0.9.3-web-wasm.zip` | any static web host | The browser build — unzip, serve the folder, open `index.html` |
...
```bash
chmod +x POM2-v0.9.3-x86_64.AppImage
./POM2-v0.9.3-x86_64.AppImage
```

If your distro no longer ships `libfuse2`, either install it or run the image
without it: `./POM2-v0.9.3-x86_64.AppImage --appimage-extract-and-run`.
...
```bash
git tag v0.9.3 && git push origin v0.9.3  # `0.9.0` without the v works too
```
