...
- **Media write by default; the write-protect is the notch ON THE DISK**
  *(2026-09-08)*. Every disk image — 5.25", 3.5", HDV, CFFA, SmartPort unit
  — comes up writable. Write-protect is a property of the disk, as on the
  real thing (a sticker over the sleeve's notch, a 3.5" tab): POM2 models it
  as the image file's **host read-only bit** (`MediaNotch.h`), which every
  loader already mounted as protected. The "Write-protected" tick in each
  media panel, the Slot Config media rows and the Disk Library's right-click
  menu (any image, mounted or not, with a lock glyph on the row) flips that
  bit through `StorageCoordinator::setMediaNotch` — file first, unlocked,
  then every mounted copy under the lock — so the protection follows the
  disk into any drive and any emulator, and nothing is persisted in
  `state.cfg`. A WOZ or a 2IMG whose header says protected stays protected
  whatever the bit says. There is **no per-drive or per-card toggle in the
  UI**: the `writeBackEnabled` flags under the leaves are only the process
  default from `MediaWritePolicy.h` (`POM2_MEDIA_WRITE_DEFAULT=protected`
  flips it, and **ctest sets it for every test** because dozens of tests boot
  the tracked images and the cards commit dirty media in their destructors —
  a test or probe that boots a tracked image must copy it first if it wants
  writes). A legacy `*_writeback = false` key is migrated once at restore:
...
**//c-class CPU + Chat Mauve rules.** The //c/+/enhanced-//e (NTSC + PAL) and //c PAL profiles have a **65C02 soldered** (`defaultCpu = CMOS`); an `cpu_mode_override = nmos` is *ignored* there (`resolveCpuMode` clamp) — forcing NMOS made their 65C02 ROMs hit KIL opcodes and freeze. The **Unenhanced PAL //e** ships NMOS (like its NTSC sibling) and accepts overrides both ways. The **Le Chat Mauve RGB** card is one of two peripherals allowed on a `noPhysicalSlots` //c (it's the rear DB-15 "Adaptateur IIc", not a slot card; the other is the CPU-socket **Mockingboard 4c** on virtual slot 3, stored under its own key `iic_expansion_card` via `pom2::slotCardSettingKey`, and dormant at $C400 until a program writes there): user-pluggable on plain //c/+ via a `{empty, Le Chat Mauve}` Slot-Config combo, and a **fixed sl7 built-in on the //c PAL profile**. Profile-forced slots are no longer persisted to `slot_N_card` on exit (quitting on //c used to clobber the user's //e card config).
...
Current release: **v0.9.3**. **Single source of truth = `CMakeLists.txt`
