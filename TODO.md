...
- ✅ **The dead cross-reference and the TransWarp ROM** *(2026-09-08,
  superseded 2026-09-16)*: `§ Disk images` → `§ 💿 ROMs and media`. The
  README then described `ae_transwarp_1.4.bin` as a name with no public
  dump known; RetroBIOS has one (CRC `afe37f55` = MAME `warprom`), POM2
  ships it, and the README says so.
...
| 13 | SmartPortHub + Sony35Drive     | Verbatim         | `apple2e.cpp:638-679`, `mac_floppy.cpp`, `flopimg.cpp:512/967/2017-2106` | 🟡 deliberate: /READY (sense `$E`) does not follow `mon_w` — the //c+ ROM waits on it after MotorOff (bug hunt #9, `iicplus_boot35`); register `$9` keeps the ROM's polarity (0 = WP), the inverse of MAME's                                                                                        |
...
| 19 | Phasor (AE — 2×VIA, 4×AY)      | Verbatim | MAME `a2bus/phasor.cpp` + AppleWin                                       | 🟢 EchoPlus mode (=7) routed as native Phasor; stereo L/R per VIA pair done (2026-08-01). 🟢 **cycle-stamped event queue** (2026-09-09): every AY store and /RESET strobe is replayed by the audio thread at its `emuCycles` stamp, the Mockingboard's design for four chips — a 511 Hz volume PWM written by the CPU rendered at 27 Hz (buffer-quantised) before, 510 Hz after; pinned `phasor_timeline`. |
...
| 23quater | TranswarpCard (key `transwarp`) | Partial-verbatim | MAME `bus/a2bus/transwarp.cpp` (R. Belmont, 363 lines; line-cited) | 🟢 **deliberate divergence**: MAME runs a SECOND W65C02 DMA-ing the Apple's bus because a MAME card cannot retime the host CPU; POM2 scales `cyclesPerFrame` and keeps the machine's own 6502 — closer to the board and free on the hot path. Register semantics, DIP defaults and the slowdown windows are verbatim. 🟢 multiplier sampled per 4096-cycle chunk (2026-09-09; per frame before — a slow window opening mid-frame left the whole frame at 3.5×, 59 657 cycles where ~22 000 were due; pinned `transwarp_chunk_sampling`); 🟢 ROM shadow: `roms/ae_transwarp_1.4.bin` ships since 2026-09-16 (it reached RetroBIOS; CRC `afe37f55` = MAME `warprom`). Pinned `transwarp_card` |
